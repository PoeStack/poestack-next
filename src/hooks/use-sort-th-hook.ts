...
import { 
    ...
    ColumnsSortingMap } from '@components/sortable-table-header';

type SortChangeHandler = (key: string | undefined, dir: SortingDirections)=>void;
...
export default function useSortableTable(
    ...
    onSortChange: SortChangeHandler): [ColumnsSortingMap, Dispatch<string>] {
     ...
     */
    useEffect(() => {
        const sortKey = Object.keys(map).find(
          (k) => map[k] !== "none");
 ...
 */
function createDefaultSortingMap(cols: SortableTableColumns): ColumnsSortingMap {
