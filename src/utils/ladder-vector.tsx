...
export class LadderVectorUtil {
  public static parse(rawVector: LadderVectorRaw): LadderVector {
    ...
    const itemKeysIndex = Object.entries(rawVector.types).find(
      (e) => e[1] === "itemKeys"
    )?.[0];
  ...
  private static executeFilter(
    ...
    if (
      include.length &&
      !filteredValues.some((item) => include.includes(item))
    ) {
      return false;
    }
    if (
      exclude.length &&
      filteredValues.some((item) => exclude.includes(`!${item}`))
    ) {
      return false;
    }
    return true;
  ...
  public static executeSearch(
    baseVector: LadderVector,
    search: LadderVectorSearch,
    ladderGroup: CustomLadderGroup | null
  ): LadderVector {
    ...
    nextVector.entires = nextVector.entires.filter((vector) => {
      if (
        !LadderVectorUtil.executeFilter(search.class, [vector.characterClass])
      ) {
        return false;
      }

      if (
        !LadderVectorUtil.executeFilter(search.skill, [vector.mainSkillKey])
      ) {
        return false;
      }
    ...
    nextVector.entires.forEach((e) => {
      nextVector.classAggregation[e.characterClass] =
        nextVector.classAggregation[e.characterClass] + 1;
      nextVector.mainSkillKeyAggregation[e.mainSkillKey ?? "na"] =
        nextVector.mainSkillKeyAggregation[e.mainSkillKey ?? "na"] + 1;
      e.itemKeys.forEach(
        (ik) =>
          (nextVector.itemKeyAggregation[ik] =
            nextVector.itemKeyAggregation[ik] + 1)
      );
    });
