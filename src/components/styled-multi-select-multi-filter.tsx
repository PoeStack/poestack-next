...
export default function StyledMultiSelectMultiFilter(
  {
    ...
    placeholder?: string;
    additionalFilters?: [{ title: string, toggle: () => void, enabled: Boolean, filterFunction: (arg0: any) => Boolean }]
  }
) {
  const [query, setQuery] = useState('')

  const filteredItems = additionalFilters ? additionalFilters.reduce((curr, filter) => {
    return filter.enabled ? curr.filter((item) => filter.filterFunction(item.name)) : curr;
  }, items) : items;
  ...
  const unselectItem = (id: any) => {
    const newSelected = selected?.filter((s: any) => s.id !== id) || []
    onSelectChange(newSelected);
  }


  return (
    <Combobox value={selected} onChange={onSelectChange} multiple>
        ...
        <div className="relative h-12 fit flex w-full cursor-default overflow-hidden rounded-lg bg-color-primary text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
          ...
          <div className='max-w-lg flex items-center overflow-x-auto overflow-y-hidden mxr-1'>
            {selected.length > 0 && (
              <>
                {
                  selected.map((el: any) => {
                    return <div key={el.id} className="hover:bg-color-secondary bg-color-primary-variant my-1 mx-2 px-2 text-white rounded flex items-center h-[70%] cursor-pointer text-center" onClick={() => unselectItem(el.id)}>
          ...
          {additionalFilters && additionalFilters.map((filter) => {
            return (
              <button
                key={filter.title}
                className={`bg-color-secondary hover:bg-color-secondary-variant py-1 px-2  border-r-2 border-color-primary font-semibold ${filter.enabled
                  ? "text-content-accent"
                  : "text-white"
                  }`}
            ...
            {cleanItems.length === 0 && query !== '' ? (
              ...
              cleanItems.map((item) => (
                <Combobox.Option
                  key={item.id}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 capitalize ml-2 ${active ? "bg-color-primary-variant " : "text-white"
                    }`
                  }
                  value={item}
                >
