...
export default function ZoomableSVG({ 
  ...
  height, 
  scaleLimit, 
  resetZoomEmitter }: ZoomableSVGProps) {

  const id = useId();
  const idRef = useRef(id);
  const slRef = useRef<[min: number, max: number]>()
  const svgRef = useRef<SVGSVGElement>(null);
  const selectedRef = useRef<d3Selection<SVGElement,undefined, null, undefined>>();
  ...
  useEffect(() => {
    if(svgRef.current){
      ...
      if(zoomBehaviorRef.current && selectedRef.current) {
          select<SVGElement,undefined>(svgRef.current).call(
  ...
  function resetZoomState() {
    if(svgRef.current && zoomBehaviorRef.current){
      ...
      select<SVGElement,undefined>(svgRef.current).transition()
        .duration(250)
        .call(zoomBehaviorRef.current.transform, zoomIdentity);
    }
  }

  function zoomListener({ transform: { x, y, k } }: { transform: ZoomTransform }) {
    setK(k);
    setX(x);
    setY(y);    
  }
