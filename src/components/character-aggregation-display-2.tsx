...
export default function CharacterAggregationDisplay2({
  ...
  let totalMatches = 0;
  const mappedRow = Object.entries(aggregation).map(([key, value]) => {
    totalMatches += value;
    return { key: key, value: value };
  })
    .filter((e) => e.key !== "na")
    .filter((e) => e.key.toLowerCase().includes(localSearchString.toLowerCase()))
    .sort((a, b) => b.value - a.value)

  const includeRows = [query].flatMap((e) => e).filter((e) => !e?.startsWith("!"));
  const excludeRows = [query].flatMap((e) => e).filter((e) => e?.startsWith("!"));
  ...
  const Row = ({ index, style }) => (
    <div
      ...
      onClick={() => { onRowClicked?.(mappedRow[index]?.key) }}
    >
      <div
        className={`
              truncate
      ...
      <div
        className={`text-right ${includeRows.includes(mappedRow[index].key)
    ...
    <>
      <div className="flex flex-col flex-1 h-full">
        <div className="grid items-center pr-2 space-y-1 text-sm capitalize truncate cursor-pointer hover:bg-color-primary ">
          {excludeRows.map((e) => e?.slice(1)).map((e) => (
            <>
              <div
                key={e}
